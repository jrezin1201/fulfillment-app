# Dot Fulfillment - Kit Manufacturing & Fulfillment App

## Project Memory & Architectural Guidelines

This is **Dot Fulfillment**, a Next.js 15+ (App Router) application for managing kit-based manufacturing and fulfillment, built using **Vertical Slice Architecture**.
...
- **projects** - Create and manage quote/SOW projects, configure kits and quantities
- **kits** - Define kit types (Bulk Fit, DTC A1c, etc.) with BOMs and sourcing
- **components** - Component library with inventory, sourcing, pricing, and vendor tracking
- **sow-generator** - (Planned) Generate professional SOW documents from project data

### 🎯 Core Business Logic

The app handles the decision tree for kit fulfillment:

1. **Is the component customer-provided?**
   - YES → Exclude from "Manufacture & Source" costs, include receiving/QA
   - NO → Proceed to step 2

2. **How many must be ordered?**
   - Calculate: (units per kit × total kits) + buffer
   - Consider MOQ and print efficiency thresholds

3. **Lead time & risk adjustment**
   - Dot-sourced: Use standard lead times, flag at-risk components
   - Customer-provided: Require delivery date, add dependency clauses

### 💰 Pricing Structure

- **Manufacture & Source** - Component costs (if Dot sources)
- **Assembly & Fulfillment** - Kitting, QA, packaging, shipping
- **Extended Kit Total** - Total per kit type
- **Total Program Cost** - Sum across all kits in project
